...
const RoadmapGenerator: React.FC<RoadmapGeneratorProps> = ({ user }) => {
  ...
  if (activeRoadmap) {
    ...
    return (
      <div className="space-y-6 md:space-y-12 animate-fade-in max-w-full overflow-hidden">
        {/* Active Roadmap Title Bar */}
        <div className="bg-white p-5 md:p-14 rounded-3xl md:rounded-[3rem] border border-slate-100 shadow-xl relative overflow-hidden">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
            <div className="space-y-3 w-full min-w-0">
              <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Active Mission</span>
              <h2 className="text-xl md:text-4xl lg:text-5xl font-black text-slate-800 tracking-tight leading-tight break-words">{activeRoadmap.topic}</h2>
            ...
            <button 
              onClick={() => { if(confirm("Start a new roadmap mission?")) setActiveRoadmap(null); }} 
              className="w-full lg:w-auto px-6 py-3 bg-slate-50 text-slate-800 rounded-xl font-black hover:bg-slate-100 transition-all text-[10px] uppercase tracking-widest shadow-sm border border-slate-200 active:scale-95 flex-shrink-0"
            >
              New Strategy
            </button>
          </div>
        ...
        {/* Schedule List - Optimized for Mobile Alignment */}
              ...
              <div key={idx} className={`group bg-white rounded-2xl md:rounded-3xl p-5 md:p-12 shadow-md border transition-all duration-300 ${isDone ? 'border-green-100 bg-slate-50/30' : 'border-slate-100'}`}>
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center space-x-3 md:space-x-6 min-w-0">
                    <div className={`w-10 h-10 md:w-16 md:h-16 rounded-xl flex-shrink-0 flex items-center justify-center font-black text-sm md:text-2xl transition-all shadow-sm ${isDone ? 'bg-green-500 text-white' : 'bg-indigo-600 text-white'}`}>
                      {item.day}
                    </div>
                    <div className="min-w-0 truncate">
                      <h3 className="text-sm md:text-2xl font-black text-slate-800 leading-tight truncate">{item.title}</h3>
                      {isDone && <span className="text-[8px] md:text-[10px] font-black text-green-600 uppercase tracking-widest">Mastered</span>}
                    </div>
                  </div>
                  <button 
                    onClick={() => handleToggleDay(item.day)}
                    className={`p-3 md:p-4 rounded-xl border-2 flex-shrink-0 transition-all active:scale-95 ${isDone ? 'bg-green-500 border-green-500 text-white' : 'bg-white border-slate-100 text-slate-200 hover:border-indigo-600 hover:text-indigo-600'}`}
                  >
                ...
                <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                  <div className="space-y-6">
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-3">
                        {item.tasks.map((task, i) => (
                          <li key={i} className="flex items-start space-x-3 text-slate-700 text-xs md:text-lg font-medium leading-relaxed">
                            <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${isDone ? 'bg-green-400' : 'bg-indigo-500'}`}></div>
                    ...
                    {item.links && item.links.length > 0 && (
                      <div className="space-y-3">
                        <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">Reference Vault</p>
                        <div className="flex flex-wrap gap-2">
                          {item.links.map((link, i) => (
                            <a 
                              ...
                              className="px-3 py-2 bg-indigo-50 text-indigo-700 rounded-lg text-[9px] font-bold flex items-center gap-1.5 hover:bg-indigo-100 border border-indigo-100 transition-colors"
                            >
                  ...
                  <div className="bg-slate-50 p-6 md:p-8 rounded-2xl border border-slate-100 h-fit">
                    <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest mb-1">Strategy</p>
                    <p className="text-sm md:text-lg text-slate-800 font-medium italic leading-relaxed break-words">"{item.revisionTip}"</p>
                  </div>
                </div>
              </div>
            );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in px-1 overflow-x-hidden">
      <div className="text-center space-y-4 px-2">
        <h2 className="text-3xl md:text-7xl font-black text-slate-900 tracking-tighter leading-tight break-words">Architect Your Path</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10">
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-5 py-3.5 rounded-xl bg-slate-50 border-2 border-transparent focus:border-indigo-600 focus:bg-white outline-none transition-all font-bold text-sm md:text-lg cursor-pointer"
            >
          ...
          <div className="space-y-2">
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Proficiency</label>
            <select
              value={form.level}
