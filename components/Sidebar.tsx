...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange }) => {
  const menuItems = [
    ...
    { id: AppTab.VOICE, label: 'Voice Companion', icon: 'M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-20a3 3 0 013 3v10a3 3 0 01-3 33 3 0 01-3-3V5a3 3 0 013-3z' },
  ...
  return (
    <aside className="w-20 lg:w-72 flex flex-col border-r border-slate-200 bg-white h-screen transition-all duration-300">
      {/* Sidebar Branding */}
      <div className="p-8 hidden lg:block">
        <div className="flex items-center space-x-3 mb-10">
          ...
          <div className="flex flex-col">
            <h1 className="text-sm font-black text-slate-900 leading-none">Rasineni's</h1>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Companion</span>
          </div>
        </div>
        <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Navigator</h2>
      </div>
      ...
      <nav className="flex-1 space-y-1 px-3 lg:px-6">
        {menuItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onTabChange(item.id)}
            className={`w-full flex flex-col lg:flex-row items-center space-y-1 lg:space-y-0 lg:space-x-4 px-2 lg:px-4 py-3 lg:py-4 rounded-2xl transition-all duration-200 group ${
              activeTab === item.id
          ...
          >
            <svg
              className={`w-5 h-5 lg:w-6 lg:h-6 transition-colors ${activeTab === item.id ? 'text-white' : 'text-slate-400 group-hover:text-indigo-500'}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <span className="text-[8px] lg:text-sm font-black tracking-tight text-center lg:text-left leading-none lg:leading-normal">
              {item.label}
            </span>
          </button>
        ))}
      ...
      <div className="p-6 border-t border-slate-100 hidden lg:block">
        <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-200/50">
          <p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Growth Phase</p>
