...
export const generateVisualConcept = async (concept: string) => {
  ...
  if (!candidates || candidates.length === 0) throw new Error("No response candidates received from Visual Studio.");
  ...
  if (!imageUrl && !explanation) throw new Error("The AI failed to sketch the visual concept. Please try a more specific topic.");
